...
describe("NodeEditorPanel", () => {
  it("renders nothing when no node is selected", () => {
    const { container } = render(
      <NodeEditorPanel node={null} onChange={vi.fn()} onClose={vi.fn()} />
    );
    expect(container).toBeEmptyDOMElement();
  });

  it("shows the node's name, description, and code", () => {
    render(<NodeEditorPanel node={node} onChange={vi.fn()} onClose={vi.fn()} />);
  ...
  it("calls onChange with an updated node when the name changes", async () => {
    ...
    render(<NodeEditorPanel node={node} onChange={onChange} onClose={vi.fn()} />);
  ...
  it("calls onClose when the close button is clicked", async () => {
    const user = userEvent.setup();
    const onClose = vi.fn();
    render(<NodeEditorPanel node={node} onChange={vi.fn()} onClose={onClose} />);
